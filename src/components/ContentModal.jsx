...
export default function ContentModal({ children, media_type, id }) {
	...
	const fetchData = async () => {
		const { data } = await axios.get(
			`https://api.themoviedb.org/3/${media_type}/${id}?api_key=e2211c12467b6a57e0ecc1fdffcdfaa9&language=en-US`
		);
		setContent(data);
	};
	...
	const fetchVideo = async () => {
		const { data } = await axios.get(
			`https://api.themoviedb.org/3/${media_type}/${id}/videos?api_key=e2211c12467b6a57e0ecc1fdffcdfaa9&language=en-US`
		);
